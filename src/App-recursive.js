...
function App() {
  const rowDimensions = ["category", "subCategory", "region"];
  ...
  const mainCategories = Array.from(new Set(sales.map(saleItem => { return saleItem[rowDimensions[0]]})));
  ...
  // For each row dimension,
    // category
        ...
        // subcategory
          // sub-subcategory
          // sub-subcategory
  // when no sub-category remains, get sale data for that category

  const mainCategory = rowDimensions[0];
  const getSalesByCategoryRecursive = (categories, rowDimensions) => {
    const dimension = rowDimensions.shift();
    const category = categories.shift();
    if (rowDimensions.length > 0) {
      return { [category]: getSalesByCategoryRecursive(categories, rowDimensions) };
    } else {
      return { [category]: getSalesByCategory(mainCategory, category, dimension) };
    }     
  }
  const mappedCategories = getSalesByCategoryRecursive(mainCategories, rowDimensions);
  console.log("🚀 ~ file: App.js:94 ~ App ~ mappedCategories:", mappedCategories)
  debugger;
